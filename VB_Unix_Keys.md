...
This document contains a process of describing the workflow for both data inspection and processing of **Veeraya Bamrung**:

## _Data Inspection_


* first, I `mkdir` VB_UNIX_Assignment/00_data_files and  go to the directory and put the assignment's data files in there 
...
* Next, I use the commands below to see how many lines and how large each file is so I can decide on a command to inspect the content in each file
...
* I also inspect how many columns are in each file using `awk`
...
For the fang file
The number of columns is huge. So I will not include the result here. However, the file contains ID, Group, JG, chromosome name on the header, and the contents are SNP details, starting at row 2

* I decided not to inspect the middle of the data; however, I used `cut` to see only a specific header from Fang et al
...
```
### _Clean the files_
* Since SNP content starts at row 2, I use `tail` to specify only the content part, cleaned the SNP files, and `cut` to only have 3 columns that the assignment asked for, I also `sort` them from column 1 as well
```
...
```
* I go to the intermediate file and inspect if the `sort` command work using `head`
```
...
* I go back to my UNIX main directory and copy maize_genotypes.txt and teosinte_genotypes.txt to the transpose folder
```
$ cp ./00_data_files/intermediate/* ./02_transponse_files
```

* I go to transponse directory and move snp that comes with the previous folder to the join folder
```
$ mv snp_pos_sorted.txt ./05_join_files
```

* I transpose the two files and send them to the intermediate folder inside the transpose folder
```
...
* So I cleaned the files by removing the head and sorted the two files
...
* I go to join the directory, make an output folder, and merge the two files, given that the second file should be maize and teosinte
    _at first, I changed the order, making teosinte and maize files came up first, because we chose from pattern in those two, and the order of the file in `join` matters. I did switch between orders. In this case, the file is the same from `wc` command_
```
$ mkdir output
...
* I go back to my main directory and copy the join files to the 03_maize_files and 04_teosinte_files_ folder
```
...
```
* I create a header file by type headers for snp file and cutting the header from the maize and teosinte files
```
$ {
...
```
* I then copied both headers to the 03_maize_files and 04_teosinte_files folders
